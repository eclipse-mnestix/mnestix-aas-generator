...
**Decision**: Apply regex-based sanitization after mapping: replace characters not in `[a-zA-Z0-9_]` with `_`, prepend `i` if the result starts with a non-letter character (e.g., digit or underscore). Log a warning when sanitization changes the value.
